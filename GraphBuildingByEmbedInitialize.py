...
Build the constituency graphs. Each virtual node is initialized by content embeddings of its connected nodes at lower level.
Use the banepar parser via spaCy. More information of the parsed can be found here,
    ...
    for i, node in enumerate(nodes):
        if len(node['children'])==1 and node['children'][0]['nodeType']=='Leaf':
            ...
            if node['children'][0]['wordIndex'] not in added_nodes:
                src_node_id = node['children'][0]['wordIndex'] + previous_token_num
            else:
